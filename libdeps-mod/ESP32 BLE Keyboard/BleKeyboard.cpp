...
BleKeyboard::BleKeyboard(std::string deviceName, std::string deviceManufacturer,
                         uint8_t batteryLevel, uint16_t currentActiveDevice)
    : hid(0),
...
void BleKeyboard::begin(void) {
    ...
    hid->reportMap((uint8_t*)_hidReportDescriptor,
...
void BleKeyboard::set_current_active_device(uint16_t currentDeviceIndex) {
    this->currentActiveDevice = currentDeviceIndex;
    ESP_LOGD(LOG_TAG, ">>>>>>>>>>>> ACTIVE DEVICE: %u ",
...
void BleKeyboard::onConnect(BLEServer* pServer) {
    ...
    this->connectedCount = pServer->getConnectedCount();
