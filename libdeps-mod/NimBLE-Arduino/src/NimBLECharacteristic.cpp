...
NimBLECharacteristic::NimBLECharacteristic(const NimBLEUUID& uuid,
                                           ...
                                           uint16_t max_len,
                                           NimBLEService* pService)
    : m_value(std::min(CONFIG_NIMBLE_CPP_ATT_VALUE_INIT_LENGTH, (int)max_len),
              max_len) {
    m_uuid = uuid;
...
void NimBLECharacteristic::removeDescriptor(NimBLEDescriptor* pDescriptor,
    ...
    pDescriptor->m_removed =
        deleteDsc ? NIMBLE_ATT_REMOVE_DELETE : NIMBLE_ATT_REMOVE_HIDE;
    NimBLEDevice::getServer()->serviceChanged();
}  // removeDescriptor
 ...
 * @return A pointer to the descriptor object or nullptr if not found.
 */
NimBLEDescriptor* NimBLECharacteristic::getDescriptorByUUID(
 ...
 */
uint16_t NimBLECharacteristic::getHandle() { return m_handle; }  // getHandle
 ...
 */
NimBLEUUID NimBLECharacteristic::getUUID() { return m_uuid; }  // getUUID
 ...
 */
size_t NimBLECharacteristic::getDataLength() { return m_value.size(); }

/**
 * @brief STATIC callback to handle events from the NimBLE stack.
 */
int NimBLECharacteristic::handleGapEvent(uint16_t conn_handle,
    ...
    NIMBLE_LOGD(LOG_TAG, "Characteristic %s %s event",
    ...
    if (ble_uuid_cmp(uuid, &pCharacteristic->getUUID().getNative()->u) == 0) {
        switch (ctxt->op) {
            case BLE_GATT_ACCESS_OP_READ_CHR: {
                ...
                // If the packet header is only 8 bytes this is a follow up of a
                // long read so we don't want to call the onRead() callback
                // again.
                if (ctxt->om->om_pkthdr_len > 8 ||
                    pCharacteristic->m_value.size() <=
                        (ble_att_mtu(desc.conn_handle) - 3)) {
 ...
 * This will maintain a vector of subscribed clients and their indicate/notify
 * status.
 */
void NimBLECharacteristic::setSubscribe(struct ble_gap_event* event) {
    ...
    if (event->subscribe.cur_notify > 0 &&
        (m_properties & NIMBLE_PROPERTY::NOTIFY)) {
    ...
    if (!event->subscribe.cur_indicate && event->subscribe.prev_indicate) {
        NimBLEDevice::getServer()->clearIndicateWait(
 ...
 */
void NimBLECharacteristic::indicate() { notify(false); }  // indicate
...
/**
 * @brief Send an indication.
 * @param[in] value A std::vector<uint8_t> containing the value to send as the
 * notification value.
 */
 ...
 * @param[in] is_notification if true sends a notification, false sends an
 * indication.
 */
void NimBLECharacteristic::notify(uint16_t currentActiveDevice,
                                  bool is_notification) {
    notify(m_value.data(), m_value.length(), currentActiveDevice,
           is_notification);
}  // notify
...
void NimBLECharacteristic::notify(const std::vector<uint8_t>& value,
                                  uint16_t currentActiveDevice,
                                  bool is_notification) {
    notify(value.data(), value.size(), currentActiveDevice, is_notification);
}  // notify
 ...
 * indication.
 */
void NimBLECharacteristic::notify(const uint8_t* value, size_t length,
                                  uint16_t currentActiveDevice,
                                  bool is_notification) {
    ...
    if (!(m_properties & NIMBLE_PROPERTY::NOTIFY) &&
        !(m_properties & NIMBLE_PROPERTY::INDICATE)) {
        NIMBLE_LOGE(LOG_TAG,
                    "<< notify-Error; Notify/indicate not enabled for "
                    "characteristic: %s",
    ...
    NIMBLE_LOGD(LOG_TAG, "Current active device: %u", currentActiveDevice);
    ...
    for (auto& it : m_subscribedVec) {
        if (it.first == currentActiveDevice) {
            ...
            if (length > _mtu) {
                NIMBLE_LOGW(LOG_TAG,
                            "- Truncating to %d bytes (maximum notify size)",
                            _mtu);
            }

            if (is_notification && (!(it.second & NIMBLE_SUB_NOTIFY))) {
                NIMBLE_LOGW(LOG_TAG,
                            "Sending notification to client subscribed to "
                            "indications, sending indication instead");
                is_notification = false;
            }

            if (!is_notification && (!(it.second & NIMBLE_SUB_INDICATE))) {
                NIMBLE_LOGW(LOG_TAG,
                            "Sending indication to client subscribed to "
                            "notification, sending notification instead");
            ...
            // We also must create it in each loop iteration because it is
            // consumed with each host call.
...
void NimBLECharacteristic::setValue(const uint8_t* data, size_t length) {
    ...
    NIMBLE_LOGD(LOG_TAG,
                ">> setValue: length=%d, data=%s, characteristic UUID=%s",
 ...
 * @param [in] vec The std::vector<uint8_t> reference to set the characteristic
 * value from.
 */
 ...
 * @param [in] pCharacteristic The characteristic that is the source of the
 * event.
 * @param [in] desc The connection description struct that is associated with
 * the peer that performed the read.
 */
void NimBLECharacteristicCallbacks::onRead(
 ...
 * event.
 * @param [in] desc The connection description struct that is associated with
 * the peer that performed the write.
 */
void NimBLECharacteristicCallbacks::onWrite(
    NimBLECharacteristic* pCharacteristic, ble_gap_conn_desc* desc) {
    NIMBLE_LOGD("NimBLECharacteristicCallbacks", "onWrite: default");
 ...
 * event.
 */
void NimBLECharacteristicCallbacks::onNotify(
    NimBLECharacteristic* pCharacteristic) {
...
void NimBLECharacteristicCallbacks::onStatus(
    NimBLECharacteristic* pCharacteristic, Status s, int code) {
